...
def _unpack_sign_bits(packed: Tensor, shape: tuple[int, ...]) -> Tensor:
    ...
    flat = np.unpackbits(
        packed.to(device="cpu", dtype=torch.uint8).numpy(),
        bitorder="little",
        count=total,
    )
...
def quantize_float_tensor(
    name: str, t: Tensor
) -> tuple[Tensor, Tensor, dict[str, object], Tensor | None, Tensor | None]:
    t32 = t.float()
...
def quantize_state_dict_int8(state_dict: dict[str, Tensor]):
    ...
    for name, tensor in state_dict.items():
        ...
        q, s, tensor_qmeta, packed_residual, residual_scale = quantize_float_tensor(
            name, t
        )
        if tensor_qmeta:
...
def dequantize_state_dict_int8(obj: dict[str, object]) -> dict[str, Tensor]:
    ...
    for name, q in obj["quantized"].items():
        ...
        if meta.get("residual") == "sign" and name in residual_bits and name in residual_scales:
            bits = _unpack_sign_bits(
                residual_bits[name], tuple(int(v) for v in meta["shape"])
            ).float()
            signs = bits.mul_(2.0).sub_(1.0)
            rs = residual_scales[name]
            if rs.ndim > 0:
                restored = restored + signs * rs.float().view(
                    signs.shape[0], *([1] * (signs.ndim - 1))
                )
            else:
...
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str]):
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            q, s, qmeta, residual_bits, residual_scale = quantize_int6_per_row(name, t)
            result[name + ".q"] = q
            result[name + ".scale"] = s
            if residual_bits is not None and residual_scale is not None:
                result[name + ".residual_bits"] = residual_bits
                result[name + ".residual_scale"] = residual_scale
            meta[name] = {"type": "int6", **qmeta}
        else:
            q, s, qmeta, residual_bits, residual_scale = quantize_float_tensor(name, t)
            result[name + ".q"] = q
            result[name + ".scale"] = s
            if residual_bits is not None and residual_scale is not None:
                result[name + ".residual_bits"] = residual_bits
                result[name + ".residual_scale"] = residual_scale
            meta[name] = {"type": "int8", **qmeta}
    return result, meta
...
def dequantize_mixed_int6(
    ...
    for name, orig in template_sd.items():
        ...
        if s.ndim > 0:
            restored = q.float() * s.float().view(q.shape[0], *([1] * (q.ndim - 1)))
        else:
            restored = q.float() * float(s.item())
        if isinstance(info, dict) and info.get("residual") == "sign":
            bits = _unpack_sign_bits(
                result[name + ".residual_bits"], tuple(int(v) for v in info["shape"])
            ).float()
            signs = bits.mul_(2.0).sub_(1.0)
            rs = result[name + ".residual_scale"]
            if rs.ndim > 0:
                restored = restored + signs * rs.float().view(
                    signs.shape[0], *([1] * (signs.ndim - 1))
                )
                ...
                restored = restored + signs * float(rs.item())
        if isinstance(info, dict) and info.get("transform") == "signed_fwht":
            restored = _invert_structured_rotation(name, restored, int(info["block_size"]))
