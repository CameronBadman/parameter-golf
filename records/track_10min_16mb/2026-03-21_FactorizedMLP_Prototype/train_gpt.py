...
class Hyperparameters:
    ...
    mlp_factor_rank = int(os.environ.get("MLP_FACTOR_RANK", 0))
...
class FactorizedLinear(nn.Module):
    def __init__(self, in_features: int, out_features: int, rank: int):
        super().__init__()
        self.rank = rank
        self.in_proj = CastedLinear(in_features, rank, bias=False)
        self.out_proj = CastedLinear(rank, out_features, bias=False)
...
class MLP(nn.Module):
    def __init__(self, dim: int, mlp_mult: float, factor_rank: int = 0):
        super().__init__()
        hidden = int(mlp_mult * dim)
        if factor_rank > 0:
            self.fc = FactorizedLinear(dim, hidden, factor_rank)
            self.proj = FactorizedLinear(hidden, dim, factor_rank)
            self.proj.out_proj._zero_init = True
        else:
...
class Block(nn.Module):
    def __init__(self, dim: int, num_heads: int, num_kv_heads: int, mlp_mult: float, rope_base: float, qk_gain_init: float, mlp_factor_rank: int = 0):
        super().__init__()
        self.attn_norm = RMSNorm()
        self.mlp_norm = RMSNorm()
        self.attn = CausalSelfAttention(dim, num_heads, num_kv_heads, rope_base, qk_gain_init)
        self.mlp = MLP(dim, mlp_mult, factor_rank=mlp_factor_rank)
...
class GPT(nn.Module):
    def __init__(
        ...
        self.blocks = nn.ModuleList(
            [
                Block(model_dim, num_heads, num_kv_heads, mlp_mult, rope_base, qk_gain_init, mlp_factor_rank=mlp_factor_rank)
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        mlp_factor_rank=args.mlp_factor_rank,
    ).to(device).bfloat16()
    ...
    log0(f"mlp_factor_rank:{args.mlp_factor_rank}")
