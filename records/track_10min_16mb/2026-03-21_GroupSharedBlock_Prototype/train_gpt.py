...
class Hyperparameters:
    ...
    shared_block_groups = int(os.environ.get("SHARED_BLOCK_GROUPS", 0))
...
class SharedBlock(nn.Module):
    ...
    def forward(self, x: Tensor, x0: Tensor, resid_mix: Tensor, attn_scale: Tensor, mlp_scale: Tensor) -> Tensor:
...
class GPT(nn.Module):
    def __init__(
        ...
        self.num_shared_blocks = num_layers if shared_block_groups <= 0 else min(shared_block_groups, num_layers)
        ...
        self.shared_blocks = nn.ModuleList(
            [
                SharedBlock(model_dim, num_heads, num_kv_heads, mlp_mult, rope_base, qk_gain_init)
                for _ in range(self.num_shared_blocks)
            ]
        )
        self.block_group_map = [(i * self.num_shared_blocks) // num_layers for i in range(num_layers)]
        ...
        self.resid_mixes = nn.Parameter(
            torch.stack(
                (
                    torch.ones(num_layers, model_dim),
                    torch.zeros(num_layers, model_dim),
                ),
                dim=1,
            ).float()
        )
    ...
    def _init_weights(self) -> None:
        ...
        num_layers = len(self.block_group_map)
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            block = self.shared_blocks[self.block_group_map[i]]
            x = block(
                x,
                x0,
                self.resid_mixes[i].to(dtype=x.dtype),
                self.attn_scales[i].to(dtype=x.dtype),
                self.mlp_scales[i].to(dtype=x.dtype),
            )
            skips.append(x)
        for i in range(self.num_decoder_layers):
            ...
            block = self.shared_blocks[self.block_group_map[layer_idx]]
            x = block(
                x,
                x0,
                self.resid_mixes[layer_idx].to(dtype=x.dtype),
                self.attn_scales[layer_idx].to(dtype=x.dtype),
                self.mlp_scales[layer_idx].to(dtype=x.dtype),
            )
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        shared_block_groups=args.shared_block_groups,
    ).to(device).bfloat16()
    ...
    log0(f"shared_block_groups:{args.shared_block_groups} shared_blocks:{base_model.num_shared_blocks}")
